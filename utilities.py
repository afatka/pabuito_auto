...
class utilities():
    ...
    def isTransformWithoutShape(self, node):
        print("the shape is visible... does it have no children?: {}".format(node))
        if cmds.listRelatives(node, shapes = True) != None:
            print("children shapes detected")
            return False
        print('No child shapes')
        return True
    ...
    def visibilityTest(self, inputNode):
        ...
        # if cmds.nodeType(testNode) == 'transform' and cmds.listRelatives(testNode, parent = 1, path=1) == None:return False
